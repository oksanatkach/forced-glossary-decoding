...
def next_token_prediction(seq, encoder_outputs):
    ...
    return next_token_scores, next_token, token_score, max_index
...
while True:
    # while stopping condition: all sequences are done
    ...
    for ind in range(len(forced_seqs)):
        ...
        if not done:
            seq = forced_seqs[ind]
            next_token_scores, next_token, token_score, max_index = next_token_prediction(seq, encoder_outputs)
    ...
    if not original_done:
        # get logits
        next_token_scores, next_token, token_score, max_index = next_token_prediction(original_seq, encoder_outputs)

        # get next forced seq score
